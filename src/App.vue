...
socket.on("delete", ({ type, id }) => {
  if (type === "room" && id) {
    ...
    context.messages = context.messages.filter((message) => message.id !== id)
})
...
<template>
  ...
  >
    <div
      class="relative w-full max-w-2xl max-h-full bg-gray-800 rounded-lg shadow p-6 text-white text-center"
    >
      <h3 class="text-xl font-semibold">Create a Room</h3>
  ...
  <!--Room joining modal-->
  <div
    v-if="joinRoomModal.open"
    class="overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 bottom-0 z-40 flex justify-center items-center w-full md:inset-0 bg bg-gray-600/50"
  >
    <div
      class="relative w-full max-w-2xl max-h-full bg-gray-800 rounded-lg shadow p-6 text-white text-center"
    >
      <h3 class="text-xl font-semibold">Join a Private Room</h3>
  ...
  <div
    v-if="usernameModal.open"
    class="overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 bottom-0 z-40 flex justify-center items-center w-full md:inset-0 bg bg-gray-600/50"
  >
    <div
      class="relative w-full max-w-2xl max-h-full bg-gray-800 rounded-lg shadow p-6 text-white text-center"
    >
      <h3 class="text-xl font-semibold">Pick a Username</h3>
      ...
      <div class="py-4 font-semibold text-gray-400 align-middle">
        <input
          ...
          type="text"
          class="w-80 py-2.5 px-5 m-1 text-sm font-medium rounded-lg border border-gray-400 bg-gray-600 outline-none text-white"
          v-model="usernameModal.username"
        />
      </div>
      ...
      <div class="flex justify-center">
        ...
        <button
          ...
          @click="
            usernameModal.open = false
            usernameModal.username = context.username
          "
        >
          Cancel
        </button>
  ...
  <div class="flex flex-col h-screen bg-gray-900 text-white p-4">
    <div
      class="p-4 px-6 bg-gray-800 flex justify-between items-center rounded-lg"
    >
      ...
      <div class="flex items-center">
        <button
          @click="toggleSounds"
          class="h-10 px-4 mr-2 rounded-lg font-bold"
          :class="
            soundsEnabled
        ...
        >
          <img
            :src="`/icons/volume${soundsEnabled ? '' : '-muted'}.svg`"
            class="h-10"
          />
        </button>

        <button
          @click="usernameModal.open = true"
    ...
    <div class="flex flex-1 overflow-hidden mt-4 relative">
      <div class="w-64 bg-gray-800 rounded-lg">
        ...
        >
          <details open v-if="context.privateRooms.length !== 0">
            <summary class="font-bold cursor-pointer">Private Rooms</summary>

            <ul class="h-full scrollbar-transparent">
              <li
                v-for="(room, index) in context.privateRooms"
                :key="index"
          ...
          <details :open="context.publicRooms.length !== 0">
            <summary class="font-bold cursor-pointer">Public Rooms</summary>

            <ul
              class="h-full scrollbar-transparent"
              v-if="context.publicRooms.length !== 0"
            >
              <li
                v-for="(room, index) in context.publicRooms"
                :key="index"
                class="my-2 rounded-lg"
              >
                <button
        ...
        <div
          class="absolute bottom-0 left-0 right-0 p-2 w-64 bg-gray-800 rounded-lg"
        >
          <button
            @click="joinRoomModal.open = true"
      ...
      <div v-if="context.roomid" class="flex-1 p-4 overflow-y-auto rounded-lg">
        <div
          v-if="context.rooms.find(({ id }) => context.roomid === id).private"
        ...
        >
          ...
          <button
            @click="leave"
            class="px-5 py-2.5 bg-red-500 hover:bg-red-600 rounded-lg text-sm font-bold"
          >
            Leave Room
          </button>
        </div>
        ...
        <ul>
          <li
            ...
            :class="
              message.highlight
                ? 'highlight' +
                  ((context.messages[index + 1]?.highlight ? ' below' : '') +
                    (context.messages[index - 1]?.highlight ? ' above' : ''))
                : ''
            "
            v-html="message.msg"
      ...
      <div
        v-else
        class="flex-1 p-4 rounded-lg flex flex-col items-center justify-center text-gray-400"
      >
        <h1>Welcome to Elliptical!</h1>
        <h3>
          {{ context.rooms.length === 0 ? "Create" : "Select" }} a room to start
          chatting...
        </h3>
      </div>
    </div>
    ...
    <div
      class="p-4 mt-2 bg-gray-800 rounded-lg text-center"
      v-if="context.roomid"
    >
      <form @submit.prevent="onSubmit" class="flex items-center">
        ...
        />
        <button
          class="px-5 py-2.5 ml-2 bg-blue-500 hover:bg-blue-600 rounded-lg text-sm font-bold"
        >
          Send
        </button>
      </form>
